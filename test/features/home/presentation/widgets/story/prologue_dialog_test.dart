...
void main() {
  ...
  setUp(() {
    ...
    when(() => layoutBloc.state).thenReturn(
      const AppLayoutState.initial(appVersion: AppVersionEnum.mobile),
    );
        ...
        child: MaterialApp(
          ...
          home: Builder(
            builder: (context) => Scaffold(
              body: TextButton(
                onPressed: () => PrologueDialog.show(context, cubit: cubit),
  ...
  testWidgets('PrologueDialog uses a mid-wide card on desk landscape', (tester) async {
    when(() => layoutBloc.state).thenReturn(
      const AppLayoutState.initial(appVersion: AppVersionEnum.desk),
    );
    ...
    // Desk landscape: ~68% of 1440 ≈ 979, capped at 1040.
