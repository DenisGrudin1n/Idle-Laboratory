...
class GameValidators {
  ...
  static ValidationResult validateCellUnlock(
    ...
    final unlockRequirement = cell.unlockRequirement;
    if (unlockRequirement == null)
      return const ValidationResult.failure('No unlock requirement defined');
    if (availableEnergy < unlockRequirement) {
      return ValidationResult.failure(
        'Insufficient energy. Required: ${unlockRequirement.formatCompact()}, Available: ${availableEnergy.formatCompact()}',
      );
    }
  ...
  static ValidationResult validateCellLevelUp(
    ...
    final levelUpRequirement = cell.nextLevelEnergyRequired;
    ...
    if (availableEnergy < levelUpRequirement) {
      return ValidationResult.failure(
        'Insufficient energy. Required: ${levelUpRequirement.formatCompact()}, Available: ${availableEnergy.formatCompact()}',
      );
    }
    return const ValidationResult.success();
  }
  ...
  static ValidationResult validatePrestige(PrestigeStateModel prestigeState) {
    if (!prestigeState.isUnlocked) {
      return ValidationResult.failure(
        'Prestige is locked. Reach ${prestigeState.currentThreshold.formatCompact()} energy to unlock',
      );
    }
  ...
  static ValidationResult validateEnergyRequirement(
    ...
    if (available < required) {
      return ValidationResult.failure(
        'Insufficient energy. Required: ${required.formatCompact()}, Available: ${available.formatCompact()}',
      );
    }
    return const ValidationResult.success();
  }
