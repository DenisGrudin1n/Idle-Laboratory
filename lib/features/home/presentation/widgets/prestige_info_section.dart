...
class PrestigeInfoSection extends StatelessWidget {
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          if (kDebugMode)
                            Row(children: [Expanded(child: _buildPrestigeButton(context, l10n, prestigeState)), SizedBox(width: 8.w), _buildResetButton(context)])
                          else
  ...
  Widget _buildMultiplierDisplay(BuildContext context, AppLocalizations l10n, PrestigeStateModel prestigeState) => Container(
        padding: EdgeInsets.all(12.w),
        ...
        child: Column(
          ...
          children: [
            Text('${l10n.prestigeMultiplier}: ${prestigeState.totalMultiplier.format(compact: true)}x',
                style: TextStyle(color: context.color.primary, fontSize: 10.sp, fontWeight: FontWeight.w500), textAlign: TextAlign.center, maxLines: 2, overflow: TextOverflow.ellipsis),
            SizedBox(height: 3.h),
            Text('+ ${prestigeState.currentMultiplier.format(compact: true)}x ${l10n.prestigeBonus}',
                style: TextStyle(color: prestigeState.isUnlocked ? context.color.green : context.color.primaryText.withValues(alpha: 0.5), fontSize: 11.sp, fontWeight: FontWeight.w600),
  ...
  Widget _buildProgressSection(BuildContext context, AppLocalizations l10n, PrestigeStateModel prestigeState) => BlocSelector<CellsBloc, CellsState, BigNumber?>(
        ...
        builder: (context, totalEnergy) {
          ...
          return Column(
            children: [
              ...
              SizedBox(height: 3.h),
              InfoRow(label: l10n.prestigeRequirement, value: prestigeState.currentThreshold.format(compact: true), valueColor: context.color.primaryText),
              SizedBox(height: 6.h),
  ...
  Widget _buildPrestigeButton(BuildContext context, AppLocalizations l10n, PrestigeStateModel prestigeState) => ActionButton(
  ...
  Widget _buildResetButton(BuildContext context) => Material(
        ...
        child: InkWell(
          onTap: () async {
            ...
            if ((confirm ?? false) && context.mounted) context.read<PrestigeBloc>().add(const PrestigeEvent.resetPrestige());
          },
          borderRadius: BorderRadius.circular(8.r),
          child: Container(
            padding: EdgeInsets.all(4.w),
            decoration: BoxDecoration(color: Colors.red.withValues(alpha: 0.2), borderRadius: BorderRadius.circular(8.r), border: Border.all(color: Colors.red.withValues(alpha: 0.5), width: 1.5.w)),
